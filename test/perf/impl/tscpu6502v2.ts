...
export default class CPU6502 {
    ...
    constructor(options: CpuOptions = {}) {
        ...
        for (let idx = 0; idx < 0x100; idx++) {
            if (ops[idx]) {
                opary[idx] = () => ops[idx][1].call(this, ops[idx][2] as any)
            } else {
                opary[idx] = () => this.unknown(idx);
            }
        }
        this.opary = opary;
    }
    ...
    /* Load Accumulator */
    lda(readFn: ReadFn) {
        this.ar = this.testNZ(readFn.call(this));
    }

    /* Load X Register */
    ldx(readFn: ReadFn) {
        this.xr = this.testNZ(readFn.call(this));
    }

    /* Load Y Register */
    ldy(readFn: ReadFn) {
        this.yr = this.testNZ(readFn.call(this));
    }

    /* Store Accumulator */
    sta(writeFn: WriteFn) {
        writeFn.call(this, this.ar);
    }

    /* Store X Register */
    stx(writeFn: WriteFn) {
        writeFn.call(this, this.xr);
    }

    /* Store Y Register */
    sty(writeFn: WriteFn) {
        writeFn.call(this, this.yr);
    }
    ...
    /* Add with Carry */
    adc(readFn: ReadFn) {
        this.ar = this.add(this.ar, readFn.call(this), /* sub= */ false);
    }

    /* Subtract with Carry */
    sbc(readFn: ReadFn) {
        this.ar = this.add(this.ar, readFn.call(this), /* sub= */ true);
    }
    ...
    inc(readAddrFn: ReadAddrFn) {
        var addr = readAddrFn.call(this, { rwm: true });
    ...
    and(readFn: ReadFn) {
        this.ar = this.testNZ(this.ar & readFn.call(this));
    }
    ...
    cmp(readFn: ReadFn) {
        this.compare(this.ar, readFn.call(this));
    }

    cpx(readFn: ReadFn) {
        this.compare(this.xr, readFn.call(this));
    }

    cpy(readFn: ReadFn) {
        this.compare(this.yr, readFn.call(this));
    }
    ...
    public dumpPage(start?: word, end?: word) {
        ...
        for (var page = start; page <= end; page++) {
            var b: number, idx: number, jdx: number;
